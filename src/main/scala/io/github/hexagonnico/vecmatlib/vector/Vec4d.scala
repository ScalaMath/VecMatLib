...
import io.github.hexagonnico.vecmatlib.{Double2, Double3, Double4}
...
case class Vec4d(x: Double, y: Double, z: Double, w: Double) extends VecDouble[Vec4d] with Double4 {
   ...
   */
  override def abs: Vec4d = Vec4d(math.abs(this.x), math.abs(this.y), math.abs(this.z), math.abs(this.w))
