...
import io.github.hexagonnico.vecmatlib.matrix.{Mat2f, Mat2x3f}
...
case class Vec2f(x: Float, y: Float) {
  ...
  def rotated(angle: Double): Vec2f = {
    ...
    val cosine = math.cos(angle).toFloat
    Vec2f(this.x * cosine - this.y * sine, this.x * sine + this.y * sine)
  }
   ...
   * @return The outer product between this vector and the given one.
   */
  def outer(v: Vec3f): Mat2x3f = Mat2x3f(
