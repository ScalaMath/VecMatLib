...
import io.github.hexagonnico.vecmatlib.vector.{Vec2f, Vec3d, Vec3f}
...
case class Mat3f(
   ...
   * @return The subtraction between this matrix and the given one.
   */
  def minus(m: Mat3f): Mat3f = this - m
   ...
   * @return The product between this matrix and the given scalar.
   */
  def multipliedBy(k: Float): Mat3f = this * k
   ...
   * @return The division between this matrix and the given scalar.
   */
  def dividedBy(k: Float): Mat3f = this / k
  ...
  /**
   * Returns the third row of this matrix as a [[Vec3f]].
   *
   ...
   */
  def *(v: Vec3f): Vec3f = Vec3f(this.row0.dot(v), this.row1.dot(v), this.row2.dot(v))
   ...
   * @return The product of this matrix by the vector with the given components.
   */
  def *(xy: Vec2f, z: Float): Vec3f = this * (xy.x, xy.y, z)
   ...
   * @return The product of this matrix by the vector with the given components.
   */
  def multiply(xy: Vec2f, z: Float): Vec3f = this * (xy, z)
   ...
   * @return The product between this matrix and the given one.
   */
  def multiply(m: Mat3f): Mat3f = this * m
   ...
   * @return The product between this matrix and the given one.
   */
  def multiply(m: Mat3x4f): Mat3x4f = this * m
  ...
  def submatrix(i: Int, j: Int): Mat2f = (i, j) match {
    ...
    case (0, 1) => Mat2f(this.m10, this.m12, this.m20, this.m22)
    case (0, 2) => Mat2f(this.m10, this.m11, this.m20, this.m21)
    case (1, 0) => Mat2f(this.m01, this.m02, this.m21, this.m22)
    case (1, 1) => Mat2f(this.m00, this.m02, this.m20, this.m22)
    case (1, 2) => Mat2f(this.m00, this.m01, this.m20, this.m21)
   ...
   */
  def inverse: Mat3f = this.adjugate / this.determinant
   ...
   */
  def lerp(to: Mat3f, weight: Float): Mat3f = Mat3f(
    ...
    vecmatlib.lerp(this.m20, to.m20, weight), vecmatlib.lerp(this.m21, to.m21, weight), vecmatlib.lerp(this.m22, to.m22, weight)
  )
   ...
   */
  def orthonormalized: Mat3f = {
    var (v0, v1, v2) = (this.col0, this.col1, this.col2)
    v1 = v1 - v1.project(v0)
    v2 = v2 - v2.project(v0) - v2.project(v1)
    Mat3f.fromColumns(v0.normalized, v1.normalized, v2.normalized)
  }
   ...
   */
  def ~=(m: Mat3f): Boolean = (this.m00 ~= m.m00) && (this.m01 ~= m.m01) && (this.m02 ~= m.m02) &&
    (this.m10 ~= m.m10) && (this.m11 ~= m.m11) && (this.m12 ~= m.m12) &&
    (this.m20 ~= m.m20) && (this.m21 ~= m.m21) && (this.m22 ~= m.m22)
   ...
   * @return True if this matrix is approximately equal to the given one, otherwise false.
   */
  def equalsApprox(m: Mat3f): Boolean = this ~= m
...
object Mat3f {
   ...
   */
  def fromRows(row0: Vec3f, row1: Vec3f, row2: Vec3f): Mat3f = Mat3f(
   ...
   */
  def fromColumns(col0: Vec3f, col1: Vec3f, col2: Vec3f): Mat3f = Mat3f(
   ...
   */
  def translation(t: Vec2f): Mat3f = this.translation(t.x, t.y)
   ...
   * @return A 3x3 rotation matrix with the given rotation.
   */
  def rotation(r: Vec3f): Mat3f = this.rotation(r.x, r.y, r.z)
   ...
   * @return A 3x3 rotation matrix with the given rotation.
   */
  def rotation(r: Vec3d): Mat3f = this.rotation(r.x, r.y, r.z)
   ...
   * @return A 3x3 rotation matrix with the given rotation.
   */
  def rotation(axis: Vec3f, angle: Double): Mat3f = {
   ...
   */
  def rotation(q: Quatf): Mat3f = {
    val s = 1.0f / q.lengthSquared
    Mat3f(
      1.0f - 2.0f * s * (q.y * q.y + q.z * q.z), 2.0f * s * (q.x * q.y - q.z * q.w), 2.0f * s * (q.x * q.z + q.y * q.w),
      2.0f * s * (q.x * q.y + q.z * q.w), 1.0f - 2.0f * s * (q.x * q.x + q.z * q.z), 2.0f * s * (q.y * q.z - q.x * q.w),
      2.0f * s * (q.x * q.z - q.y * q.w), 2.0f * s * (q.y * q.z + q.x * q.w), 1.0f - 2.0f * s * (q.x * q.x + q.y * q.y)
    )
  }
  ...
  implicit class MultiplicationExtender(val self: Float) extends AnyVal {
     ...
     */
    def *(v: Mat3f): Mat3f = v * self
  }

  /**
   * Implicit conversion from [[Mat3f]] to [[Mat3d]].
   */
  implicit val mat3fToMat3d: Mat3f => Mat3d = v => v.toDouble
}
