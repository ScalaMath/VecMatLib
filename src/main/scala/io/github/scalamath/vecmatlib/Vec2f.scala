...
case class Vec2f(x: Float, y: Float) {
   ...
   */
  def round: Vec2f = Vec2f(this.x.round.toFloat, this.y.round.toFloat)
   ...
   */
  def bezierInterpolate(to: Vec2f, control: Vec2f, t: Float): Vec2f = Vec2f(
   ...
   */
  def bezierDerivative(to: Vec2f, control: Vec2f, t: Float): Vec2f = Vec2f(
   ...
   */
  def bezierInterpolate(to: Vec2f, control1: Vec2f, control2: Vec2f, t: Float): Vec2f = Vec2f(
   ...
   */
  def bezierDerivative(to: Vec2f, control1: Vec2f, control2: Vec2f, t: Float): Vec2f = Vec2f(
...
object Vec2f {
   ...
   *
   * This method is equivalent to `Vec2f(cos(angle), sin(angle))` or `Vec2f.Right.rotated(angle)`.
   *
   ...
   */
  def fromAngle(angle: Double): Vec2f = Vec2f(math.cos(angle).toFloat, math.sin(angle).toFloat)
  ...
  implicit class MultiplicationExtender(val self: Float) extends AnyVal {
     ...
     */
    def /(v: Vec2f): Vec2f = v.inverse * self
  }
